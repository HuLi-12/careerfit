...
@dataclass
class ResumeProfile:
    basic_info: Dict[str, str] = field(default_factory=lambda: {'name':'','target_position':'','location':'','contact':''})
    education: List[Dict[str, Any]] = field(default_factory=list)
    work_experience: List[Dict[str, Any]] = field(default_factory=list)
    project_experience: List[ProjectExperienceItem] = field(default_factory=list)
    skills: Dict[str, List[str]] = field(default_factory=lambda: {'programming':[],'frameworks':[],'tools':[],'database':[],'domain_knowledge':[],'soft_skills':[],'all':[]})
