...
def main():
    ...
    a=p.parse_args()
    text=Path(a.input).read_text(encoding='utf-8') if a.input else (sys.stdin.read() if not sys.stdin.isatty() else '')
    out=CareerFitRouter().run(text,a.format)
    if a.output:
        Path(a.output).write_text(out,encoding='utf-8'); print(f'已生成报告：{a.output}')
    else: print(out)
